...
class KoaLa {
  ...
  async loadServices () {
    ...
    await this.loadAndAssign('../services', this.app.context, Service => new Service())
  }

  async loadAndAssign (path, assignTo, loader) {
    try {
      ...
      filesList.forEach((file) => {
        ...
        if (name) {
          assignTo[name] = loader ? loader(loadedModule) : loadedModule(this)
        } else {
